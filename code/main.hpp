...
serialize_config(void){
    ...
    for(s32 i=0; i < pm->csv_profile_count; ++i){
        ...
        arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at,
                "name=%s\x1B date=%s\x1B amount=%s\x1B description=%s\x1B format=%s\x1B format_idx=%i\x1B\n",
    ...
    arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at, "csv_profile_idx=%i\x1B\n", pm->csv_profile_idx);

    // tabs selected
    arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at, "#tabs_selected\n");
    arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at,
                          "quarter_tab_idx=%i\x1B biannual_tab_idx=%i\x1B\n", pm->quarter_tab_idx, pm->biannual_tab_idx);

    // collapsables
    arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at, "#collapsables\n");
    arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at,
                          "draw_month_plan=%i\x1B draw_quarter_plan=%i\x1B draw_biannual_plan=%i\x1B draw_annual_plan=%i\x1B\n",
    ...
    arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at,
                          "xpos=%i\x1B ypos=%i\x1B width=%i\x1B height=%i\x1B maximized=%i\x1B\n",
    ...
    if(window.maximized){
        ...
        arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at,
                              "left=%i\x1B top=%i\x1B right=%i\x1B bottom=%i\x1B\n",
    ...
    arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at, "show_tooltips=%i\x1B\n", show_tooltips);

    // transaction_year
    arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at, "#year\n");
    arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at,
            "year_idx=%i\x1B month_tab_idx=%i\x1B\n", pm->year_idx, pm->month_tab_idx);
...
serialize_year(Year* year){
    ...
    if(year->transaction_count){
        ...
        for(s32 m_idx=0; m_idx < Month_Count; ++m_idx){
            ...
            arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at, "muted=%i\x1B\n", month->muted);

            Transaction* t = month->transactions;
            for(s32 t_idx = 0; t_idx < month->transaction_count; ++t_idx){
                t = t->next;
                arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at,
                                      "date=%s\x1B amount=%s\x1B description=%s\x1B selection=%s\x1B muted=%i\x1B\n",
...
serialize_budget(void){
    ...
    arena->at += snprintf((char*)arena->base + arena->at, arena->size - arena->at, "budget=%s\x1B\n", pm->budget);
